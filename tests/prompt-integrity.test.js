...
describe('CORE_IDENTITY — always-banned food override', () => {
  ...
  it('contains profile update tag format for strictness', () => {
    expect(CORE_IDENTITY).toContain('[STRICTNESS_UPDATE:');
  });

  it('contains profile update tag format for community', () => {
    expect(CORE_IDENTITY).toContain('[COMMUNITY_UPDATE:');
  });
